...
public final class RatPoly {
    ...
    private static void sortedInsert(List<RatTerm> lst, RatTerm newTerm) {
        if(newTerm.isZero()) {
            ...
            // inv: newTerm != 0 && lst.get(0)... lst.get(term). Find the
            // index based on descending exponent order in order to insert
            // the new terms.
            for(RatTerm term : lst) {
                if(term.getExpt() < newTerm.getExpt()) {
                lst.add(lst.indexOf(term), newTerm);
                return;
            } else if(term.getExpt() == newTerm.getExpt()) {
                if((new RatTerm(term.getCoeff().add(newTerm.getCoeff()), term.getExpt())).isZero()) {
                    lst.remove(term);
                } else {
                    lst.set(lst.indexOf(term), new RatTerm(term.getCoeff().add(newTerm.getCoeff()), term.getExpt()));
                }
                return;
            }
        }
    ...
    // a private helper method that iterates through all the terms of RatPoly
    private ArrayList<RatTerm> iterate() {
            ArrayList<RatTerm> temp = new ArrayList<RatTerm>();
            // inv: temp.get(0) ... temp.get(i). For every value at temp, there will be a RatTerm that will
            // have a coefficient and exponent.
            for (int i = 0; i < terms.size(); i++) {
     ...
     */
        ...
        public RatPoly negate() {
            ...
            ArrayList<RatTerm> temp = iterate();
    ...
    public RatPoly add(RatPoly p) {
        ...
        ArrayList<RatTerm> temp = p.iterate();
        // inv: rt.get(0) ... temp.get(i). For every value at rt, the method,
        // sorted insert will run.
    ...
    public RatPoly mul(RatPoly p) {
        ...
        // inv: term.get(0) ... temp.get(i). For every term, the scale in coefficients and increment in exponents
        // will be used to find the product of the arrayList of terms.
        for (RatTerm term : terms) {
            ArrayList<RatTerm> temp = p.iterate();
            scaleCoeff(temp, term.getCoeff());
            incremExpt(temp, term.getExpt());
    ...
    public RatPoly div(RatPoly p) {
        ...
        RatPoly r = new RatPoly(this.iterate());
        if (p.terms.size() == 0 || (this.isNaN() || p.isNaN())) {
            return new RatPoly(RatTerm.NaN);

        }
        // inv: Keep interating if the remainder is not zero or remainder's degree is greater than
        // or equal to the divisor's degree,
        while(r.terms.size() != 0 && r.degree() >= p.degree()) {
            RatNum one = r.terms.get(0).getCoeff().div(p.terms.get(0).getCoeff());
            RatPoly two = new RatPoly(new RatTerm(one, r.degree() - p.degree()));
            q = q.add(two);
            r = r.sub(two.mul(p));
        }
        q.checkRep();
